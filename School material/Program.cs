...
class Program{
	...
	static void Page(byte number,string go_to_page,string wait_for_page){//download material from page
		...
		byte i=2,j,content_number;int _;string subject;List<byte> haves;
		foreach(string table_value in table_items){
			if(subjects.IsMatch(table_value)){//my subject
				_=table_value.IndexOf('-');
				subject=table_value.Substring(_+2,table_value.Length-_-14);
				content_number=Convert.ToByte(table_value.Substring(8,_-9));
